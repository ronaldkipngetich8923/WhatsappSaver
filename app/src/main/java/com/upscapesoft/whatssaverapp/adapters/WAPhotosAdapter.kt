...
class WAPhotosAdapter(var activity: Activity, var fragment: WAPhotosFragment) :
    RecyclerView.Adapter<WAPhotosAdapter.ViewHolderKlasse>() {
        ...
        init {
            ...
            iv_selected.setOnClickListener {
                ...
                if (!itemStateArray[adapterPosition, false]) {
                    ...
                    WAStatusActivity.filePathsPhotosChecked!![adapterPosition] = "1"
                } else {
                    iv_selected.isChecked = false
                    itemStateArray.put(adapterPosition, false)
                    WAStatusActivity.filePathsPhotosChecked!![adapterPosition] = "0"
                }
                fragment.checkFAB()
            }
        }
    ...
    override fun onBindViewHolder(viewHolderKlasse: ViewHolderKlasse, i: Int) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            viewHolderKlasse.iv_thumbnail.setImageBitmap(
                (viewHolderKlasse.itemView.context as WAStatusActivity).getBitmapOnAndroidQ(
                    Uri.parse(
                        WAStatusActivity.filePathsPhotos!![i]
                    )
                )
            )
        } else {
        ...
        viewHolderKlasse.itemView.setOnClickListener { v: View? ->
            ...
            if (AdController.adCounter == AdController.adDisplayCounter) {
                ...
                intent.putExtra("imagesPho", WAStatusActivity.filePathsPhotos!![i])
    ...
    override fun getItemCount(): Int {
        return WAStatusActivity.filePathsPhotos!!.size
    }
