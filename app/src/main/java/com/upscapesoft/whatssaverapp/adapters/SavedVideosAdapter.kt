...
class SavedVideosAdapter(var activity: Activity, var fragment: SavedVideosFragment) :
    RecyclerView.Adapter<SavedVideosAdapter.ViewHolderKlasse>() {
        ...
        init {
            ...
            iv_selected.setOnClickListener {
                ...
                if (!itemStateArray[adapterPosition, false]) {
                    ...
                    SavedStatusActivity.filePathsVideosChecked!![adapterPosition] = "1"
                } else {
                    iv_selected.isChecked = false
                    itemStateArray.put(adapterPosition, false)
                    SavedStatusActivity.filePathsVideosChecked!![adapterPosition] = "0"
                }
                fragment.checkFAB()
            }
        }
    ...
    override fun onBindViewHolder(viewHolderKlasse: ViewHolderKlasse, i: Int) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            Glide.with(viewHolderKlasse.iv_thumbnail.context)
                .load(SavedStatusActivity.filePathsVideos!![i]).into(viewHolderKlasse.iv_thumbnail)
        } else {
            viewHolderKlasse.iv_play.visibility = View.VISIBLE
            Glide.with(viewHolderKlasse.iv_thumbnail.context).load(
                getVideoThumbnail(
                    File(
                        SavedStatusActivity.filePathsVideos!![i]
                    )
                )
        ...
        viewHolderKlasse.itemView.setOnClickListener { v: View? ->
            ...
            if (AdController.adCounter == AdController.adDisplayCounter) {
                ...
                intent.putExtra("videosVid", SavedStatusActivity.filePathsVideos!![i])
    ...
    override fun getItemCount(): Int {
        return SavedStatusActivity.filePathsVideos!!.size
    }
