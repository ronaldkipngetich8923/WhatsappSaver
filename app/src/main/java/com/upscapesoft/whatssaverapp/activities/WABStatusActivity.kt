...
class WABStatusActivity : AppCompatActivity(), PickiTCallbacks {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_wabstatus)
        activity = this

        iniApp()
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            val sharedPreferences = getSharedPreferences(packageName, 0)
            if (sharedPreferences.getString("pathWB", "")!!.length != 0) {
                uri = Uri.parse(sharedPreferences.getString("pathWB", ""))
                loadFragments()
            } else {
                startActivity(Intent(this@WABStatusActivity, WABPermissionsActivity::class.java))
            }
        } else {
            if (ActivityCompat.checkSelfPermission(
                    this@WABStatusActivity,
    ...
    private fun loadFragments() {
        ...
        tabLayout!!.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                ...
                AdController.showInterAd(this@WABStatusActivity, null, 0)
            }
    ...
    private fun setupViewPager(viewPager: ViewPager?) {
        ...
        adapter.addFragment(WABPhotosFragment(), "Photos")
        adapter.addFragment(WABVideosFragment(), "Videos")
    ...
    fun getTabViewUn(pos: Int): View {
        val v = LayoutInflater.from(this@WABStatusActivity).inflate(R.layout.custom_tab_lay, null)
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == Constants.REQ_CODE_EXTERNAL_STORAGE_PERMISSION && grantResults.size > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
            Utils.startAppWhatsappBusiness(activity!!)
        }
    }
    ...
    private fun openWhatsAppBusiness() {
        ...
        if (whatsappBusinessInstalledOrNot("com.whatsapp.w4b")) {
            ...
            try {
                ...
                Toast.makeText(applicationContext, "Please install WhatsApp Business", Toast.LENGTH_LONG).show()
            ...
            Toast.makeText(applicationContext, "WhatsApp Business not Installed", Toast.LENGTH_SHORT).show()
